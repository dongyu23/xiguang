...
export '../../../app/providers.dart' show fragmentRepositoryProvider;
import '../domain/fragment.dart';
import '../domain/fragment_repository.dart';

final fragmentsProvider =
    AsyncNotifierProvider<FragmentsNotifier, List<Fragment>>(
  FragmentsNotifier.new,
);

class FragmentsNotifier extends AsyncNotifier<List<Fragment>> {
  @override
  ...
  Future<void> _refreshFromRemote(
    FragmentRepositoryContract repo, {
  ...
  Future<Fragment> captureWithResult({
    required String text,
    required String emotion,
    required List<String> tags,
    List<String> mediaUrls = const [],
    ...
    state = const AsyncLoading<List<Fragment>>().copyWithPrevious(state);
    try {
      ...
      state = AsyncError<List<Fragment>>(error, stackTrace)
  ...
  void _invalidateDerived() {
    // Timeline and island providers watch fragmentsProvider directly. Keeping
    // invalidation local avoids initializing unrelated feature providers from
    // a capture mutation (and makes this controller deterministic in tests).
  }
