...
type Repository interface {
	...
	InsertRefreshToken(ctx context.Context, userID int64, tokenHash, deviceInfo string, expiresAt time.Time) (int64, error)
	FindRefreshUserID(ctx context.Context, tokenHash string) (int64, error)
	RotateRefreshToken(ctx context.Context, oldTokenHash, newTokenHash string, expiresAt time.Time) (int64, int64, error)
	DeviceSessionActive(ctx context.Context, userID, tokenID int64) (bool, error)
	ListDevices(ctx context.Context, userID int64) ([]domain.DeviceSession, error)
	RevokeDevice(ctx context.Context, userID, tokenID int64) (bool, error)
}
...
func (r *PG) CreateUser(ctx context.Context, username, passwordHash, nickname string) (domain.User, error) {
	...
	err = tx.QueryRow(ctx, `INSERT INTO users(username,password_hash,nickname) VALUES($1,$2,$3)
		RETURNING id, public_id::text, username, nickname, avatar_key, ai_enabled, ai_consent_accepted_at, privacy_mode, created_at`,
		username, passwordHash, nickname).Scan(&user.ID, &user.PublicID, &user.Username, &user.Nickname, &user.AvatarKey, &user.AIEnabled, &user.AIConsentAcceptedAt, &user.PrivacyMode, &user.CreatedAt)
	if err != nil {
		return domain.User{}, err
	}
...
func (r *PG) FindByUsername(ctx context.Context, username string) (domain.User, error) {
	var user domain.User
	err := r.db.QueryRow(ctx, `SELECT id, public_id::text, username, nickname, avatar_key, ai_enabled, ai_consent_accepted_at, privacy_mode, created_at, password_hash
		FROM users WHERE username=$1 AND deleted_at IS NULL`, username).
		Scan(&user.ID, &user.PublicID, &user.Username, &user.Nickname, &user.AvatarKey, &user.AIEnabled, &user.AIConsentAcceptedAt, &user.PrivacyMode, &user.CreatedAt, &user.PasswordHash)
	return user, err
}

func (r *PG) FindByID(ctx context.Context, id int64) (domain.User, error) {
	var user domain.User
	err := r.db.QueryRow(ctx, `SELECT id, public_id::text, username, nickname, avatar_key, ai_enabled, ai_consent_accepted_at, privacy_mode, created_at
		FROM users WHERE id=$1 AND deleted_at IS NULL`, id).
		Scan(&user.ID, &user.PublicID, &user.Username, &user.Nickname, &user.AvatarKey, &user.AIEnabled, &user.AIConsentAcceptedAt, &user.PrivacyMode, &user.CreatedAt)
	return user, err
}
...
func (r *PG) UpdateUser(ctx context.Context, id int64, params domain.UpdateUserParams) (domain.User, error) {
	...
	err := r.db.QueryRow(ctx, `UPDATE users
		SET nickname=COALESCE(NULLIF($2,''), nickname), avatar_key=COALESCE(NULLIF($3,''), avatar_key),
		    ...
		    ai_consent_accepted_at=CASE WHEN COALESCE($5,false) THEN now() ELSE ai_consent_accepted_at END,
		    privacy_mode=$6, updated_at=now()
		WHERE id=$1 AND deleted_at IS NULL
		RETURNING id, public_id::text, username, nickname, avatar_key, ai_enabled, ai_consent_accepted_at, privacy_mode, created_at`,
		id, params.Nickname, params.AvatarKey, params.AIEnabled, params.AIConsentAccepted, params.PrivacyMode).
		Scan(&user.ID, &user.PublicID, &user.Username, &user.Nickname, &user.AvatarKey, &user.AIEnabled, &user.AIConsentAcceptedAt, &user.PrivacyMode, &user.CreatedAt)
	return user, err
}
...
func (r *PG) DeleteUser(ctx context.Context, userID int64) error {
	tx, err := r.db.Begin(ctx)
	if err != nil {
		return err
	}
	defer tx.Rollback(ctx)
	tag, err := tx.Exec(ctx, `UPDATE users SET deleted_at=now(), updated_at=now()
...
func (r *PG) InsertRefreshToken(ctx context.Context, userID int64, tokenHash, deviceInfo string, expiresAt time.Time) (int64, error) {
	...
	err := r.db.QueryRow(ctx, `INSERT INTO refresh_tokens(user_id, token_hash, device_info, expires_at)
		VALUES($1,$2,$3,$4) RETURNING id`, userID, tokenHash, deviceInfo, expiresAt).Scan(&tokenID)
	return tokenID, err
}
...
func (r *PG) RotateRefreshToken(ctx context.Context, oldTokenHash, newTokenHash string, expiresAt time.Time) (int64, int64, error) {
	tx, err := r.db.Begin(ctx)
	if err != nil {
		return 0, 0, err
	}
	...
	err = tx.QueryRow(ctx, `UPDATE refresh_tokens
		...
		RETURNING user_id, COALESCE(device_info,'')`, oldTokenHash).Scan(&userID, &deviceInfo)
	if err != nil {
		return 0, 0, err
	}
	var tokenID int64
	if err := tx.QueryRow(ctx, `INSERT INTO refresh_tokens(user_id, token_hash, device_info, expires_at)
		VALUES($1,$2,$3,$4) RETURNING id`, userID, newTokenHash, deviceInfo, expiresAt).Scan(&tokenID); err != nil {
		return 0, 0, err
	}
	if err := tx.Commit(ctx); err != nil {
		return 0, 0, err
...
func (r *PG) DeviceSessionActive(ctx context.Context, userID, tokenID int64) (bool, error) {
	var active bool
	err := r.db.QueryRow(ctx, `SELECT EXISTS(
		SELECT 1 FROM refresh_tokens
		WHERE id=$1 AND user_id=$2 AND revoked_at IS NULL AND expires_at > now()
	)`, tokenID, userID).Scan(&active)
	return active, err
}

func (r *PG) ListDevices(ctx context.Context, userID int64) ([]domain.DeviceSession, error) {
	rows, err := r.db.Query(ctx, `SELECT id, COALESCE(device_info,''), created_at, expires_at
	...
	defer rows.Close()
	items := []domain.DeviceSession{}
	for rows.Next() {
		var item domain.DeviceSession
		if err := rows.Scan(&item.ID, &item.DeviceInfo, &item.CreatedAt, &item.ExpiresAt); err != nil {
			return nil, err
		}
...
func (r *PG) RevokeDevice(ctx context.Context, userID, tokenID int64) (bool, error) {
	tag, err := r.db.Exec(ctx, `UPDATE refresh_tokens SET revoked_at=now()
