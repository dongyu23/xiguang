-- AI assistance: only confirmed artifacts are retained long term.
...
CREATE INDEX IF NOT EXISTS idx_ai_artifacts_user
  ON ai_artifacts(user_id, updated_at DESC) WHERE deleted_at IS NULL;
...
CREATE INDEX IF NOT EXISTS idx_island_groups_user
  ON island_groups(user_id, updated_at DESC) WHERE deleted_at IS NULL;
