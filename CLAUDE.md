...
对应产品逻辑：
1. **隙**：用户把生活缝隙中的瞬间轻轻放进来
2. **线**：光片沿时间自然流动，形成可回看的时间河流
3. **织**：用户或系统发现光片之间的隐秘联系（不作为一级导航，而是上下文动作）
4. **屿**：反复出现的主题、情绪和场景沉淀为个人脉络

### 底部导航结构（v0.2）

| 导航栏 | 用户心智 | 核心任务 | MVP |
|-------|---------|---------|-----|
| **隙** | 轻轻记一下 | 快速捕光、低压力记录 | ✅ 必做 |
| **捕光按钮**（全局悬浮） | 随时放进去 | 全局快速记录 | ✅ 必做，不占导航栏，在隙/线/屿页面可见 |
| **线** | 回看过去的光 | 时间河流、旧光回访、筛选查看 | ✅ 必做 |
| **屿** | 看见个人脉络 | 主题星点、小岛、光片墙 | ✅ 必做 |
| **我的** | 管理边界 | 账号、数据、同步、隐私、组件开关 | ✅ 必做 |

注意：「织」不作为一级导航。织线发生在光片详情、时间回看、小宇宙主题中的上下文动作。

### MVP 功能决策（P0/P1/P2 分级）

**P0 — 必做主线**

| 功能 | 所属模块 | 程度 |
|-----|---------|------|
| 用户注册/登录 | 我的/系统 | 用户名+密码 |
| 捕光入口 | 隙 | App 内入口（不做桌面小组件、快捷指令） |
| 文字记录 | 隙 | 输入一句话或短文本 |
| 图片上传 | 隙 | 拍照 + 相册导入 |
| 自动时间戳 | 隙 | 捕光后自动记录时间 |
| 光片详情 | 线/详情 | 查看/编辑/软删除 |
...
| 主题星点 | 屿 | 同一主题出现 3 次后生成 |
| 小岛卡片 | 屿 | 同一主题出现 5 次后形成 |
| 同步状态/基础数据管理 | 我的 | 账号信息、本地/云端同步状态、关于隙光 |
| 全局悬浮捕光按钮 | 系统 | 在隙/线/屿页面可见，不在"我的"页显示 |

**P1 — 建议 MVP 做（亮点功能）**

| 功能 | 所属模块 | 程度 |
|-----|---------|------|
| 旧光回访 | 线 | 根据标签/情绪/日期轻提示相似旧光，**先规则实现，不需要 AI** |
| 轻织线 | 线/详情 | 系统基于规则推荐候选关系（同标签/同情绪/同时段/同岛），用户一键确认或忽略 |
| 录音记录 | 隙 | 符合多模态定位，若开发紧张可后置 |
| 阶段高频词 | 线/屿 | 近期高频标签/主题词展示，轻量版 |
| 小岛详情页 | 屿 | 名称/出现次数/首次出现/最近出现/常见情绪/代表光片/时间分布 |
| 光片墙 | 屿 | 图片光片瀑布流或柔和网格 |
| 情绪密度 | 线 | 一周内情绪色点分布，不做复杂心理分析 |
| 白噪音可选组件 | 我的/隙/屿 | 默认隐藏或从设置页开启 |

**P2 / MVP 暂不做**

| 功能 | 原因 |
|-----|------|
| 拖拽星点建立连线 | 技术复杂，用户使用频率不确定 |
| 完整个人星图 | 视觉实现成本高 |
| 3D 星空 / 海 / 房间自由切换 | 容易喧宾夺主，第一版用静态概念图示意 |
| 深织线（手动选择目标光片+关系类型） | 可作为后续增强 |
| AI 自动织线 | 后置，必须在用户确认下进行 |
| AI 柔光建议卡 | 后期 |
| AI 图片氛围分析 | 用户敏感，后置 |
| AI 推荐"那一刻的光" | 需要 AI 能力与隐私边界，后置 |
| 柔光整理 | 后期，预留按钮和 UI 入口 |
| 月度报告 | 后置 |
| 桌面小组件 | 平台适配成本高 |
| 快捷指令 | 后置 |
...
**A. 轻织线（P1 — 建议 MVP 必做）**
系统给出候选关系，用户只需做轻决策。
- 规则来源：同标签、同情绪、同时段、同媒介、同主题岛、关键词重复
- 提示文案："这束光和 3 天前那束有一点像，要不要织在一起？"
- 按钮：「织在一起」「暂时不用」
...
**C. AI 织线（后期）**
AI 根据多条光片内容发现隐性关系，但必须由用户确认。MVP 不做完整 AI 自动织线，只预留接口和按钮。
...
- ❌ 不做 3D 星空/海/房间自由切换（第一版用静态概念图）
- ❌ 不做拖拽星点建连线、完整个人星图
- ❌ 不做 AI 自动织线/AI 柔光建议卡/AI 图片氛围分析
- ❌ 不做复杂白噪音场景（不常驻首页，作为可选组件）
- ❌ MVP 不做柔光整理（预留按钮和 UI 入口）
...
| v0.2 | MVP 可用版 | 捕光→入线→回看→旧光提示→轻织线→形成小岛——闭环跑通 |
| v0.3 | 内测优化版 | 完善小宇宙视图、标签聚合、主题卡片、AI 关联建议、视觉动效 |
| v1.0 | 公开展示版 | 形成可用于比赛、路演或作品集展示的完整作品 |

### 功能优先级顺序

1. **先做轻记录**（隙 — 捕光入口最低成本能跑通）
2. **再做时间线**（线 — 让用户回看，日期分组、筛选）
3. **再做规则化旧光回访和轻织线**（不需要 AI，用可解释规则推荐）
4. **再做岛屿生长**（屿 — 主题星点、小岛沉淀）
5. **最后加入克制的 AI 柔光总结**（预留按钮，MVP 不实现）
  ...
  routes: [
    StatefulShellRoute.indexedStack(
      ...
      branches: [
        ...
        // Tab 4: 我的
        StatefulShellBranch(routes: [
          GoRoute('/mine', builder: (_, __) => MinePage()),
          GoRoute('/settings', builder: (_, __) => SettingsPage()),
        ]),
      ],
    ),
    // 非 Tab 页面（全屏）
...
- 交互原则：用户应当可以在最短路径内完成：打开 App → 输入一句话 / 上传一张图 → 点击捕光 → 完成。情绪、标签都不能成为保存前的必填项。
...
织线在以下入口作为上下文功能出现：
- **光片详情页底部**："和另一束光织在一起"（P1 深织线入口）
- **线中的旧光回访卡**："这束光和 X 天前有一点像，要不要织在一起？"（P1 轻织线）
- **屿详情页**：同一主题下的光片可建立关系
- **柔光整理结果页**（后期）：AI 输出"可选织线建议"

**A. 轻织线（P1 — 建议 MVP 必做）**
- 系统基于可解释规则推荐候选关系（同标签/同情绪/同时段/同媒介/同主题岛/关键词重复）
- 提示文案："这束光和 3 天前那束有一点像，要不要织在一起？"
- 按钮：「织在一起」「暂时不用」
...
- 关系类型：回声/伏笔/余震/平行宇宙/小小救命/潮汐/旧光/自定义
- MVP 不做拖拽星点建立连线、不做完整个人星图

**C. AI 织线（后期）**
- AI 根据多条光片内容发现隐性关系，必须由用户确认
- MVP 不做，预留接口和按钮
...
| 触发条件 | 状态变化 | 用户可见文案 |
|---------|---------|------------|
| 同一标签出现第 3 次 | → 主题星点 | "这个主题开始发光了。" |
| 同一标签出现第 5 次 | → 小岛 | "'图书馆'长成了一座小岛。" |
| 某主题很久未出现 | → 静默 | "这座小岛安静了一阵子。" |
| 静默主题再次出现 | → 旧光点亮 | "旧光被重新点亮了。" |

### 我的页

- 账号信息：昵称、头像
- 同步状态：显示本地/云端同步状态
- 数据管理：删除账号或删除全部记录的基础入口
- 关于隙光：产品介绍、隐私说明
- 组件开关：白噪音、AI 辅助等可选组件的开关
- 一句话："这里决定你的边界。"
...
- AI 推荐"那一刻的光"
- 柔光整理（三种 AI 模式：轻轻命名/帮我织线/不解释我）
- AI 隐性关联发现 / AI 自动织线
- AI 图片氛围分析
- 拖拽星点建立连线（织线上下文交互用简单选择替代）
- 完整个人星图（可视化实现成本高，P2）
- 3D 星空 / 海 / 房间自由切换（第一版用静态概念图）
- 桌面小组件、快捷指令
- 涂鸦、短视频多模态输入
- 隐私设置、AI 开关
- 数据导出与备份
